...
// 添加标志变量
let isApplyingRangeStyle = false;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 监听范围样式应用事件
    document.addEventListener('rangeStyleApplied', function(event) {
        // 当范围样式被应用时，暂时禁用整体样式更新
        const column = event.detail.column;
        const cards = document.querySelectorAll('.name-card');
        cards.forEach(card => {
            const textElement = card.querySelector(`.${column}-line`);
            if (textElement) {
                textElement.dataset.hasRangeStyle = 'true';
            }
        });
    });
});
...
function updateIfDataLoaded() {
    if (window.excelData && window.excelData.length > 0) {
        // 检查是否有范围样式
        const hasRangeStyle = document.querySelector('.has-range-style') !== null;
        ...
        // 如果有范围样式，重新应用范围样式
        if (hasRangeStyle) {
            const nameLine = document.querySelector('.name-line.has-range-style');
            if (nameLine) {
                // 获取当前范围样式设置
                const start = parseInt(document.getElementById('nameRangeStart').value);
                const end = parseInt(document.getElementById('nameRangeEnd').value);
                const fontSize = parseInt(document.getElementById('nameRangeFontSize').value);
                const isBold = document.getElementById('nameRangeBoldBtn').classList.contains('active');
    ...
    cards.forEach(card => {
        ...
        ['number', 'name', 'unit'].forEach(element => {
            ...
            if (config.gradient && config.gradient !== 'none') {
                applyGradientToElement(elementLine, config.gradient);
            }
            
            if (config.borderStyle) {
                const style = config.borderStyle;
                const color = config.borderColor;
                const width = config.borderWidth;
                
                if (style === 'all') {
                    elementLine.style.border = `${width}px solid ${color}`;
                } else if (style === 'bottom') {
                    elementLine.style.borderBottom = `${width}px solid ${color}`;
                } else if (style === 'top') {
                    elementLine.style.borderTop = `${width}px solid ${color}`;
                } else if (style === 'left') {
                    elementLine.style.borderLeft = `${width}px solid ${color}`;
                } else if (style === 'right') {
                    elementLine.style.borderRight = `${width}px solid ${color}`;
                }
            }
...
function updateCardTextStyles() {
    ...
    cards.forEach(card => {
        ['number', 'name', 'unit'].forEach(element => {
            ...
            // 更新元素样式
            updateElementTextStyles(card, element);
        });
    });
}

// 生成固定的名片卡片
function generateFixedNameCards() {
    // 更新非文本相关的样式
    updateNonTextStyles();
...
function updateElementBorders(card, element) {
    ...
    elementLines.forEach(line => {
        ...
        if (style === 'all') {
            ...
            // 分别渲染四边
            const borderConfig = window.styleConfig[element + 'Border'];
            line.style.borderTop = borderConfig && borderConfig.top ? `${width}px solid ${color}` : 'none';
            line.style.borderBottom = borderConfig && borderConfig.bottom ? `${width}px solid ${color}` : 'none';
            line.style.borderLeft = borderConfig && borderConfig.left ? `${width}px solid ${color}` : 'none';
            line.style.borderRight = borderConfig && borderConfig.right ? `${width}px solid ${color}` : 'none';
        } else if (style === 'bottom') {
...
function updateElementTextStyles(card, element) {
    ...
    // 检查元素是否有范围样式
    if (elementLine.dataset.hasRangeStyle === 'true') {
        // 保存当前的范围样式元素
        const rangeStyledElements = Array.from(elementLine.querySelectorAll('.range-styled'));
        
        // 应用文本样式到父元素
        if (config.textColor) {
            elementLine.style.color = config.textColor;
        }
        
        if (config.fontFamily) {
            elementLine.style.fontFamily = `"${config.fontFamily}", sans-serif`;
        }
        
        if (config.fontStyle) {
            elementLine.style.fontStyle = config.fontStyle;
        }
        
        if (config.textDecoration) {
            elementLine.style.textDecoration = config.textDecoration;
        }
        
        if (config.strokeWidth && config.strokeWidth !== '0') {
            elementLine.style.webkitTextStroke = `${config.strokeWidth}px ${config.strokeColor || '#000000'}`;
        } else {
            elementLine.style.webkitTextStroke = 'none';
        }
        
        if (config.shadowX !== undefined && config.shadowY !== undefined && config.shadowBlur !== undefined) {
            const shadowColor = config.shadowColor || '#000000';
            elementLine.style.textShadow = `${config.shadowX}px ${config.shadowY}px ${config.shadowBlur}px ${shadowColor}`;
        }
        
        if (config.skewAngle && config.skewAngle !== '0') {
            elementLine.style.transform = `skew(${config.skewAngle}deg)`;
        } else {
            elementLine.style.transform = 'none';
        }

        // 确保范围样式元素保持其样式
        rangeStyledElements.forEach(rangeElement => {
            // 只保留字体大小和粗体样式
            const fontSize = rangeElement.style.fontSize;
            const fontWeight = rangeElement.style.fontWeight;
            rangeElement.style.cssText = `font-size: ${fontSize} !important; font-weight: ${fontWeight} !important;`;
        });
    } else {
        // 如果没有范围样式，直接应用所有文本样式
        if (config.textColor) {
        ...
        if (config.fontSize) {
            elementLine.style.fontSize = `${config.fontSize}px`;
        }
        
        if (config.fontWeight) {
            elementLine.style.fontWeight = config.fontWeight;
        }
